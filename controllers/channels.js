const { Channel, Brand, Sequelize } = require('../sequelize')
...
const get_parent_id_list = async (data_id, parent_id_list) => {
    let data = await Channel.findOne({ where: { id: data_id } });
...
const update_tree_path = async (inserted_id) => {
    ...
    await Channel.update({ tree_path }, { where: { id: inserted_id } })
}
...
const get_data = async (req_query, is_download_excel) => {
    ...
    let query_builder_options = {
        ...
        include: [
            (Object.keys(parent_where_cause).length > 0) ? { model: Channel, as: 'parent', where: parent_where_cause } : { model: Channel, as: 'parent' },
            (Object.keys(brand_where_cause).length > 0) ? { model: Brand, where: brand_where_cause } : { model: Brand },
        ]
    }

    const data = await Channel.findAndCountAll(query_builder_options)
...
const Self = {
    ...
    create: async function (req, res) {
        const { body } = req
        const data = await Channel.create(body)
        if (data) {
    ...
    index: async function (req, res) {
        const channels = await get_data(req.query);
    ...
    update: async function (req, res) {
        ...
        const data = await Channel.update(body, { where: { id } })
    ...
    getfilterlist: async function (req, res) {
        ...
        if (filterValue !== undefined) {
            if (column === 'name' || column === 'description') {
                ...
                const data = await Channel.findAll(query_builder_options)
                for (const item of data) {
                    let push_item = { ...item.dataValues };
