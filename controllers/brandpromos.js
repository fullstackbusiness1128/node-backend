const { Pos, Brand, BrandPromos, BrandPromosPos, Static, Sequelize } = require('../sequelize')
...
const get_data = async (req, is_download_excel) => {
    ...
    let query_builder_options = {
        ...
        include: [
            ...
            { model: BrandPromosPos, include: [Pos] },
        ],
    }
    const data = await BrandPromos.findAndCountAll(query_builder_options)
...
const get_data_item = async function (dataId) {
    const data = await BrandPromos.findOne({
        where: { id: dataId },
        include: [
            Brand,
            {
...
const Self = {
    ...
    upsert: async function (req, res) {
        ...
        let data = null;
        if (body) {
            // let filtered_data = await BrandPromos.findOne({ where: { brandId: body.brandId } });
            // let dataId = id;
            // if (filtered_data) {
            ...
            if (id) {
                ...
                data = await BrandPromos.update(body, { where: { id: id } });
            } else {
                data = await BrandPromos.create(body);
            }
        }
    ...
    getfilterlist: async function (req, res) {
        ...
        if (filterValue !== undefined) {
            if (column === 'description') {
                ...
                const data = await BrandPromos.findAll({
                    where: where_cause,
                    order: [order_cause],
                })
                ...
                const data = await BrandPromos.findAll(query_builder_options)
                for (const item of data) {
                    let push_item = { ...item.dataValues };
    ...
    delete: async function (req, res) {
        const { id } = req.params
        let filtered_data = await BrandPromos.findOne({ where: { id } });
        if (filtered_data) {
            let staticId = filtered_data.pdfFile;
            if (staticId) {
                await staticController.deleteFile(staticId);
            }
            await BrandPromos.destroy({ where: { id } })
        }
    ...
    copydata: async function (req, res) {
        ...
        if (id && brandIds) {
            ...
            let filtered_data = await BrandPromos.findOne({
                where: { id },
                include: [
                    BrandPromosPos
                ]
            ...
            if (filtered_data) {
                ...
                let brandPromosData = _.omit(copydata, ["id", "brandId", "brand_promos_pos"]);
                let pos_data = copydata["brand_promos_pos"];
                if (pos_data.length > 0) {
                    pos_data = pos_data.map(el => el.dataValues.posId);
                }
                console.log(brandPromosData);
                console.log(pos_data);
                retVal.success = true;
                retVal.message = "Success !";
                for (const newBrandId of brandIds) {
                    let search = await BrandPromos.count({
                    ...
                    if (search === 0) {
                        ...
                        let inserted = await BrandPromos.create(createData);
                        if (inserted) {
                            for (const posId of pos_data) {
                                await BrandPromosPos.create({
                                    brandPromosId: inserted.id,
                                    posId,
                                })
                            }
                        }
    ...
    downloadexcelposes: async function (req, res) {
        ...
        let pos_list = [];
        if (dataItem && dataItem.brand_promos_pos && dataItem.brand_promos_pos.length > 0) {
            pos_list = dataItem.brand_promos_pos.map((el) => {
    ...
    uploadexcelIdposperbrand: async function (req, res) {
        try {
            const { brandPromosId } = req.params;
            ...
            readXlsxFile(path + req.file.filename).then(async (rows) => {
                ...
                let count = 0;
                const failedRows = [];
                await BrandPromosPos.destroy({ where: { brandPromosId } });
                for (const row of rows) {
                    ...
                    const is_new = await BrandPromosPos.create(data_detail)
                    if (is_new) {
                        count++;
                    }
                    else {
